...
const server = http.createServer((req, res) => {
  if (req.method === "GET") {
    ...
    if (req.url === "/status") {
      ...
      let clientCount = 0;
      rooms.forEach(room => clientCount += room.clients.length);
...
wss.on("connection", ws => {
  ...
  ws.on("message", message => {
    ...
    if (type === "input") {
      ...
      roomObj.state[1].facing =
        roomObj.state[1].x < roomObj.state[2].x ? 1 : -1;
      roomObj.state[2].facing =
        roomObj.state[2].x < roomObj.state[1].x ? 1 : -1;
      ...
      if (roomObj.state[1].hp === 0 || roomObj.state[2].hp === 0) {
        ...
        if (roomObj.state.score[1] < 2 && roomObj.state.score[2] < 2) {
          roomObj.state[1] = { x:150, y:350, hp:100, facing:1, punching:false };
          roomObj.state[2] = { x:650, y:350, hp:100, facing:-1, punching:false };
        }
      }
    }
